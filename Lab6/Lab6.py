...
min_opyt_rab = {"Артём": 4, "Игорь": 15, "Лев": 2, "Александр": 0, "Дмитрий": 10, "Варвара": 14, "Антонина": 5,"Полина": 8, "Арина": 11, "Павел": 6,"Татьяна": 3,
                    "София": 7, "Елизовета": 9, "Константин": 1, "Максим": 11, "Иван": 13}
costs_dict = {0: 0, 1: 2000, 2: 3000, 3: 4000, 4: 5000, 5: 5500, 6: 5600, 7: 5700, 8:5800, 9: 5900, 10:6400, 11: 6600, 12: 6650, 13:6700, 14: 6750, 15:7000} # Задаем цены на найм сотрудников в зависимости от их опыта работы
def sort_key(vacan):# Возвращает кортеж из двух значений: суммы опыта работы сотрудников на занимаемой должности и
...
#-------------------------№1-----------------------------
    ...
    print('Общее колличество вариантов', (math.factorial(16) / (math.factorial(14) * math.factorial(16))))
#--------------------------№2---------------------------
    ...
    while budget < 10000:
        budget = int(input('\nВведите натуральное число больше 10000:\n'))
    good_sotr1 = []
    good_sotr2 = []
    good_sotr3 = []
    good_sotr4 = []
    ...
    if good_sotr1:
        good_sotr1.sort(key=sort_key)  # сортировка по сумме умений и сумме цен
        best_trio = ' - '.join(good_sotr1[0][0])
        total_skill = good_sotr1[0][2]
        total_cost = good_sotr1[0][1]
        print(f'Мы нашли {len(good_sotr1)} подходящих комбинаций сотрудников тимлидеров, ' f'которые удовлетворяют вашим требованиям.\n' 
    ...
    if good_sotr2:
        good_sotr2.sort(key=sort_key)  # сортировка по сумме умений и сумме цен
        best_trio = ' - '.join(good_sotr2[0][0])
        total_skill = good_sotr2[0][2]
        total_cost = good_sotr2[0][1]
        print(f'Мы нашли {len(good_sotr2)} подходящих комбинаций сотрудников проджек-менеджеров, ' f'которые удовлетворяют вашим требованиям.\n'
            f'Наиболее подходящей комбинацией является: {best_trio}.\n' f'Общая стоимость этой комбинации: {total_cost} рублей. \n'
            f'Общая сумма умений всех сотрудников в этой комбинации: {total_skill} баллов.\n')
    if good_sotr3:
        good_sotr3.sort(key=sort_key)  # сортировка по сумме умений и сумме цен
        best_trio = ' - '.join(good_sotr3[0][0])
        total_skill = good_sotr3[0][2]
        total_cost = good_sotr3[0][1]
        print(f'Мы нашли {len(good_sotr3)} подходящих комбинаций сотрудников синьёров, ' f'которые удовлетворяют вашим требованиям.\n'
                f'Наиболее подходящей комбинацией является: {best_trio}.\n' f'Общая стоимость этой комбинации: {total_cost} рублей. \n'
                f'Общая сумма умений всех сотрудников в этой комбинации: {total_skill} баллов.\n')
    if good_sotr4:
        good_sotr4.sort(key=sort_key)  # сортировка по сумме умений и сумме цен
        best_trio = ' - '.join(good_sotr4[0][0])
        total_skill = good_sotr4[0][2]
        total_cost = good_sotr4[0][1]
        print(f'Мы нашли {len(good_sotr4)} подходящих комбинаций сотрудников мидлов, ' f'которые удовлетворяют вашим требованиям.\n'
                f'Наиболее подходящей комбинацией является: {best_trio}.\n' f'Общая стоимость этой комбинации: {total_cost} рублей. \n'
                f'Общая сумма умений всех сотрудников в этой комбинации: {total_skill} баллов.\n')
    if good_sotr5:
        good_sotr5.sort(key=sort_key)  # сортировка по сумме умений и сумме цен
        best_trio = ' - '.join(good_sotr5[0][0])
        total_skill = good_sotr5[0][2]
        total_cost = good_sotr5[0][1]
        print(f'Мы нашли {len(good_sotr5)} подходящих комбинаций сотрудников юниоров, ' f'которые удовлетворяют вашим требованиям.\n'
                f'Наиболее подходящей комбинацией является: {best_trio}.\n' f'Общая стоимость этой комбинации: {total_cost} рублей. \n'
                f'Общая сумма умений всех сотрудников в этой комбинации: {total_skill} баллов.\n')
        bol = int(input("Нужны все варианты? (да - введите 1, нет - введите 0): "))
        if bol == 1:
            print(f"Список остальных комбинаций вакансий в которых сумма умений больше минимального уровня (был задан {skill1}):")
